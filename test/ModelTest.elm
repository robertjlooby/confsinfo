  ...
  suite
    ...
    , test "should show conference if it has all included tags"
        <| let
            ...
            conference2 =
              { blankConference | tags = [ Agile, DotNet ] }

            conference3 =
              { blankConference | tags = [ DotNet, Ruby ] }

            conference4 =
              { blankConference | tags = [ Agile, DotNet, Ruby ] }
    ...
    , test "initialize includes all tags from their string versions"
        <| let
            tags =
              [ ( "", [ ( Excluded Agile, "1" ), ( Excluded DotNet, "2" ) ] ), ( "", [ ( Excluded Ruby, "3" ) ] ) ]

            newTags =
              [ ( "", [ ( Included Agile, "1" ), ( Excluded DotNet, "2" ) ] ), ( "", [ ( Included Ruby, "3" ) ] ) ]
    ...
    , test "update IncludePastEvents can uninclude past events"
        <| let
            ...
            model =
              { blankModel | includePastEvents = True }

            newModel =
              update (IncludePastEvents False) model
           in
    ...
    , test "update IncludePastEvents can include past events"
        <| let
            ...
            model =
              { blankModel | includePastEvents = False }

            newModel =
              update (IncludePastEvents True) model
           in
    ...
    , test "update SetCurrentDate sets the currentDate"
        <| let
            ...
            model =
              { blankModel | currentDate = ( 2016, Jan, 1 ) }

            newModel =
              update (SetCurrentDate 1446595200000) model
