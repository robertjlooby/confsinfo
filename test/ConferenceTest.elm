...
claims =
    suite "Model"
        ...
        , claim "cfpStatus is False if cfpEndDate is Nothing"
            `that` (\date -> cfpStatus date { blankConf | cfpStartDate = Just date })
            `is` (\_ -> ( Closed, Nothing ))
            `for` { generator = randomDaTuple
                  , shrinker = daTupleShrinker
                  }
        , claim "cfpStatus is (Open, Just cfpEndDate) if cfpEndDate == currentDate"
        ...
        , claim "compare' sorts by name if the start date is the same"
            `that` (\( d, n1, n2 ) ->
                        compare' { blankConf | name = n1, startDate = d }
